...
@section('title', 'Wisata Jambi | Approved')
			...
			<div class="col-12 grid-margin">
				<div class="card">
					<div class="card-body">
						...
						<h4 class="card-title text-success">Pengajuan Disetujui</h4>
							...
							<table class="table">
								...
								<tbody>
									...
									<tr>
										...
										<td> {{ date('Y-m-d H:i', strtotime($h->created_at)) }} </td>
										<td> 
											<a href="{{ route('admin.rm', $h->id) }}" class="btn btn-info btn-sm">Lihat</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('admin.rm.delete', $h->id) }}" 
											onclick="event.preventDefault();
											if(confirm('Anda yakin ingin menghapus entry ini?')) {
												$(this).find('form').submit();
											}
											">
											Hapus
											<form method="post" action="{{ route('admin.rm.delete', $h->id) }}">@csrf</form>
											</a>
										</td>
									</tr>
									@endforeach
									...
									<tr>
										...
										<td> {{ date('Y-m-d H:i', strtotime($h->created_at)) }} </td>
										<td> 
											<a href="{{ route('admin.tani', $h->id) }}" class="btn btn-info btn-sm">Lihat</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('admin.tani.delete', $h->id) }}" 
											onclick="event.preventDefault();
											if(confirm('Anda yakin ingin menghapus entry ini?')) {
												$(this).find('form').submit();
											}
											">
											Hapus
											<form method="post" action="{{ route('admin.tani.delete', $h->id) }}">@csrf</form>
											</a>
										</td>
									</tr>
									@endforeach
									...
									<tr>
										...
										<td> {{ date('Y-m-d H:i', strtotime($h->created_at)) }} </td>
										<td> 
											<a href="{{ route('admin.umkm', $h->id) }}" class="btn btn-info btn-sm">Lihat</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('admin.umkm.delete', $h->id) }}" 
											onclick="event.preventDefault();
											if(confirm('Anda yakin ingin menghapus entry ini?')) {
												$(this).find('form').submit();
											}
											">
											Hapus
											<form method="post" action="{{ route('admin.umkm.delete', $h->id) }}">@csrf</form>
											</a>
										</td>
									</tr>
									@endforeach

									@foreach($kendaraan as $h)
									<tr>
										<td>
										...
										<td> {{ date('Y-m-d H:i', strtotime($h->created_at)) }} </td>
										<td> 
											<a href="{{ route('admin.kendaraan', $h->id) }}" class="btn btn-info btn-sm">Lihat</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('admin.kendaraan.delete', $h->id) }}" 
											onclick="event.preventDefault();
											if(confirm('Anda yakin ingin menghapus entry ini?')) {
												$(this).find('form').submit();
											}
											">
											Hapus
											<form method="post" action="{{ route('admin.kendaraan.delete', $h->id) }}">@csrf</form>
											</a>
										</td>
									</tr>
									@endforeach
