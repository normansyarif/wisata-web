			...
			<div class="col-12 grid-margin">
				<div class="card">
					<div class="card-body">
						...
						<h4 class="card-title text-success">Daftar Homestay</h4>
						<div style="float: right; margin-bottom: 20px"><a href="{{ route('homestay.add') }}" class="btn btn-primary"><i class="mdi mdi-plus"></i> Tambah</a></div>
								...
								<tbody>
									...
									<tr>
										<td>
											...
											@if($i->foto != null)
											<img src="{{ asset('uploads/homestay/thumbs/' . $i->foto) }}" class="mr-2" alt="image" />
											@else
										...
										<td> {{ $i->kisaran_harga }} </td>
										<td> {{ $i->kamar_tersedia }} </td>
										<td> {{ $i->wilayah->nama }} </td>

										@if($i->status == 'pending')
										<td><label class="badge badge-gradient-warning">In progress</label></td>
										<td> 
											<a href="{{ route('homestay.edit', $i->id) }}" class="btn btn-success btn-sm">Edit</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('homestay.delete', $i->id) }}" 
											...
											">
											Hapus
											<form method="post" action="{{ route('homestay.delete', $i->id) }}">@csrf</form>
											</a>
										</td>
