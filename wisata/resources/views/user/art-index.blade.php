			...
			<div class="col-12 grid-margin">
				<div class="card">
					<div class="card-body">
						...
						<h4 class="card-title text-success">Kelompok Seni</h4>
						<div style="float: right; margin-bottom: 20px"><a href="{{ route('art.add') }}" class="btn btn-primary"><i class="mdi mdi-plus"></i> Tambah</a></div>
								...
								<tbody>
									...
									<tr>
										<td>
											...
											@if($i->foto != null)
											<img src="{{ asset('uploads/art/thumbs/' . $i->foto) }}" class="mr-2" alt="image" />
											@else
										...
										<td> {{ $i->jenis_kesenian }} </td>
										<td> {{ $i->wilayah->nama }} </td>

										@if($i->status == 'pending')
										<td><label class="badge badge-gradient-warning">In progress</label></td>
										<td> 
											<a href="{{ route('art.edit', $i->id) }}" class="btn btn-success btn-sm">Edit</a> 
											<a class="btn btn-danger btn-sm" href="{{ route('art.delete', $i->id) }}" 
											...
											">
											Hapus
											<form method="post" action="{{ route('art.delete', $i->id) }}">@csrf</form>
											</a>
										</td>
